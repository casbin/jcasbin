...
public class CoreEnforcer {
  ...
  /**
   * loadModel reloads the model from the model CONF file. Because the policy is attached to a
   * model, so the policy is invalidated and needs to be reloaded by calling LoadPolicy().
   */
  ...
  /**
   * enableEnforce changes the enforcing state of Casbin, when Casbin is disabled, all access will
   * be allowed by the enforce() function.
   *
  ...
  /**
   * enableAutoSave controls whether to save a policy rule automatically to the adapter when it is
   * added or removed.
   *
  ...
  /**
   * enableAutoBuildRoleLinks controls whether to save a policy rule automatically to the adapter
  ...
  /**
   * enforce use a custom matcher to decide whether a "subject" can access a "object" with the
   * operation "action", input parameters are usually: (matcher, sub, obj, act), use model matcher
   * by default when matcher is "" or null.
   *
  ...
  private boolean enforce(String matcher, Object... rvals) {
    ...
    boolean result =
        eft.mergeEffects(model.model.get("e").get("e").value, policyEffects, matcherResults);
  ...
  private boolean validateEnforceSection(String section, Object... rvals) {
    int expectedParamSize =
        getModel().model.entrySet().stream()
            ...
            .orElseThrow(
                () ->
                    new CasbinMatcherException(
                        "Could not find " + section + " definition in model"))
            .getValue()
            .tokens
            .length;
    ...
    if (rvals.length != expectedParamSize) {
      Util.logPrintfWarn(
          "Incorrect number of attributes to check for policy (expected {} but got {})",
          expectedParamSize,
          rvals.length);
  ...
  /**
   * Invalidate cache of compiled model matcher expression. This is done automatically most of the
   * time, but you may need to call it explicitly if you manipulate directly Model.
   */
