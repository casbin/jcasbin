...
public class DistributedEnforcer extends SyncedEnforcer {
  ...
  /**
   * DistributedEnforcer initializes an enforcer with a model file, a policy file and an enable log
   * flag.
   *
   * @param modelPath the path of the model file.
   * @param policyFile the path of the policy file.
  ...
  /**
   * AddPolicySelf provides a method for dispatcher to add authorization rules to the current
   * policy. The function returns the rules affected and error.
   *
   * @param shouldPersist
   ...
   */
  public List<List<String>> addPolicySelf(
      BooleanSupplier shouldPersist, String sec, String ptype, List<List<String>> rules) {
  ...
  /**
   * RemovePolicySelf provides a method for dispatcher to remove policies from current policy. The
   * function returns the rules affected and error.
   *
   * @param shouldPersist
   * @param sec the section, "p" or "g".
   * @param ptype the policy type, "p", "p2", .. or "g", "g2", ..
   ...
   * @return succeeds or not.
   */
  public List<List<String>> removePolicySelf(
      BooleanSupplier shouldPersist, String sec, String ptype, List<List<String>> rules) {
    if (shouldPersist.getAsBoolean()) {
      try {
        if (adapter instanceof BatchAdapter) {
  ...
  /**
   * RemoveFilteredPolicySelf provides a method for dispatcher to remove an authorization rule from
   * the current policy, field filters can be specified. The function returns the rules affected and
   * error.
   *
   * @param shouldPersist
   * @param sec the section, "p" or "g".
   ...
   * @return succeeds or not.
   */
  public List<List<String>> removeFilteredPolicySelf(
    ...
    List<List<String>> effects =
        this.model.removeFilteredPolicyReturnsEffects(sec, ptype, fieldIndex, fieldValues);
  ...
  /**
   * UpdatePolicySelf provides a method for dispatcher to update an authorization rule from the
   * current policy.
   *
   * @param shouldPersist
   * @param sec the section, "p" or "g".
   ...
   * @return succeeds or not.
   */
  public boolean updatePolicySelf(
      BooleanSupplier shouldPersist,
      String sec,
      String ptype,
      List<String> oldRule,
