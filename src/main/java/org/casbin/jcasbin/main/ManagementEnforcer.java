...
public class ManagementEnforcer extends InternalEnforcer {
  ...
  /**
   * addPolicies adds authorization rules to the current policy. If the rule already exists, the
   * function returns false for the corresponding rule and the rule will not be added. Otherwise the
   * function returns true for the corresponding rule by adding the new rule.
   *
  ...
  /**
   * addNamedPolicies adds authorization rules to the current named policy. If the rule already
   * exists, the function returns false for the corresponding rule and the rule will not be added.
   ...
   * @return succeeds or not.
   */
  public boolean updateNamedGroupingPolicy(
  ...
  /**
   * getPermittedActions returns all valid actions to specific object for current subject. At
   * present, the execution efficiency of this method is not high. Please avoid calling this method
   * frequently.
   *
   ...
   * @return the index of a specific element. If the above two example parameters are passed in, it
   *     will return 2. <tt>-1</tt> if the element does not exist.
   */
