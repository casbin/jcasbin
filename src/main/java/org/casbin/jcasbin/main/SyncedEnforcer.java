...
public class SyncedEnforcer extends Enforcer {
  ...
  /**
   * savePolicy saves the current policy (usually after changed with Casbin API) back to
   * file/database.
   */
  @Override
  ...
  /**
   * enforce decides whether a "subject" can access a "object" with the operation "action", input
   * parameters are usually: (sub, obj, act).
   *
   * @param rvals the request needs to be mediated, usually an array of strings, can be class
   *     instances if ABAC is used.
  ...
  /**
   * enforceWithMatcher use a custom matcher to decide whether a "subject" can access a "object"
   * with the operation "action", input parameters are usually: (matcher, sub, obj, act), use model
   * matcher by default when matcher is "" or null.
   *
   ...
   *
   * @return all the subjects in "p" policy rules. It actually collects the 0-index elements of "p"
   *     policy rules. So make sure your subject is the 0-index element, like (sub, obj, act).
   ...
   *
   * @return all the objects in "p" policy rules. It actually collects the 1-index elements of "p"
   *     policy rules. So make sure your object is the 1-index element, like (sub, obj, act).
   *     Duplicates are removed.
   */
  @Override
   ...
   * @return all the objects in policy rules of the ptype type. It actually collects the 1-index
   *     elements of the policy rules. So make sure your object is the 1-index element, like (sub,
   *     obj, act). Duplicates are removed.
   */
  @Override
   ...
   *
   * @return all the actions in "p" policy rules. It actually collects the 2-index elements of "p"
   *     policy rules. So make sure your action is the 2-index element, like (sub, obj, act).
   *     Duplicates are removed.
   */
  @Override
   ...
   *
   * @param ptype the policy type, can be "p", "p2", "p3", ..
   * @return all the actions in policy rules of the ptype type. It actually collects the 2-index
   *     elements of the policy rules. So make sure your action is the 2-index element, like (sub,
   *     obj, act). Duplicates are removed.
   */
  @Override
   ...
   *
   * @return all the roles in "g" policy rules. It actually collects the 1-index elements of "g"
   *     policy rules. So make sure your role is the 1-index element, like (sub, role). Duplicates
   *     are removed.
   */
  @Override
   ...
   * @return all the subjects in policy rules of the ptype type. It actually collects the 0-index
   *     elements of the policy rules. So make sure your subject is the 0-index element, like (sub,
   *     obj, act). Duplicates are removed.
   */
  @Override
  ...
  /**
   * getFilteredPolicy gets all the authorization rules in the policy, field filters can be
   * specified.
   *
  ...
  /**
   * getFilteredNamedPolicy gets all the authorization rules in the named policy, field filters can
   * be specified.
   *
   * @param ptype the policy type, can be "p", "p2", "p3", ..
   * @param fieldIndex the policy rule's start index to be matched.
   ...
   */
  @Override
  public List<List<String>> getFilteredNamedPolicy(
      String ptype, int fieldIndex, String... fieldValues) {
    try {
      READ_WRITE_LOCK.readLock().lock();
  ...
  /**
   * getFilteredGroupingPolicy gets all the role inheritance rules in the policy, field filters can
   * be specified.
   *
   * @param fieldIndex the policy rule's start index to be matched.
   * @param fieldValues the field values to be matched, value "" means not to match this field.
  ...
  /**
   * getFilteredNamedGroupingPolicy gets all the role inheritance rules in the policy, field filters
   * can be specified.
   *
   * @param ptype the policy type, can be "g", "g2", "g3", ..
   * @param fieldIndex the policy rule's start index to be matched.
   ...
   */
  @Override
  public List<List<String>> getFilteredNamedGroupingPolicy(
      String ptype, int fieldIndex, String... fieldValues) {
    try {
      READ_WRITE_LOCK.readLock().lock();
  ...
  /**
   * addPolicy adds an authorization rule to the current policy. If the rule already exists, the
   * function returns false and the rule will not be added. Otherwise the function returns true by
   * adding the new rule.
   *
   * @param params the "p" policy rule, ptype "p" is implicitly used.
  ...
  /**
   * AddNamedPolicy adds an authorization rule to the current named policy. If the rule already
   * exists, the function returns false and the rule will not be added. Otherwise the function
  ...
  /**
   * removeFilteredPolicy removes an authorization rule from the current policy, field filters can
   * be specified.
   *
   * @param fieldIndex the policy rule's start index to be matched.
   * @param fieldValues the field values to be matched, value "" means not to match this field.
  ...
  /**
   * removeFilteredNamedPolicy removes an authorization rule from the current named policy, field
  ...
  /**
   * addGroupingPolicy adds a role inheritance rule to the current policy. If the rule already
   * exists, the function returns false and the rule will not be added. Otherwise the function
   * returns true by adding the new rule.
   *
   * @param params the "g" policy rule, ptype "g" is implicitly used.
  ...
  /**
   * addNamedGroupingPolicy adds a named role inheritance rule to the current policy. If the rule
   * already exists, the function returns false and the rule will not be added. Otherwise the
   * function returns true by adding the new rule.
   *
   * @param ptype the policy type, can be "g", "g2", "g3", ..
   * @param params the "g" policy rule.
   * @return succeeds or not.
  ...
  /**
   * removeFilteredGroupingPolicy removes a role inheritance rule from the current policy, field
   * filters can be specified.
   *
   * @param fieldIndex the policy rule's start index to be matched.
   * @param fieldValues the field values to be matched, value "" means not to match this field.
  ...
  /**
   * removeFilteredNamedGroupingPolicy removes a role inheritance rule from the current named
  ...
  /**
   * addPermissionForUser adds a permission for a user or role. Returns false if the user or role
   * already has the permission (aka not affected).
   *
   * @param user the user.
   * @param permission the permission, usually be (obj, act). It is actually the rule without the
   *     subject.
  ...
  /**
   * deletePermissionForUser deletes a permission for a user or role. Returns false if the user or
   * role does not have the permission (aka not affected).
   *
   * @param user the user.
   * @param permission the permission, usually be (obj, act). It is actually the rule without the
   *     subject.
  ...
  /**
   * deletePermissionsForUser deletes permissions for a user or role. Returns false if the user or
   * role does not have any permissions (aka not affected).
   *
   * @param user the user.
   * @return succeeds or not.
   */
   ...
   * @return the permissions, a permission is usually like (obj, act). It is actually the rule
   *     without the subject.
   */
  @Override
  ...
  /**
   * addRoleForUserInDomain adds a role for a user inside a domain. Returns false if the user
   * already has the role (aka not affected).
   *
   * @param user the user.
   * @param role the role.
   * @param domain the domain.
  ...
  /**
   * deleteRoleForUserInDomain deletes a role for a user inside a domain. Returns false if the user
   * does not have the role (aka not affected).
   *
   * @param user the user.
   * @param role the role.
   * @param domain the domain.
   ...
   *
   * <p>getRolesForUser("alice") can only get: ["role:admin"]. But getImplicitRolesForUser("alice")
  ...
  /**
   * getImplicitPermissionsForUser gets implicit permissions for a user or role. Compared to
   ...
   *
   * <p>getPermissionsForUser("alice") can only get: [["alice", "data2", "read"]]. But
