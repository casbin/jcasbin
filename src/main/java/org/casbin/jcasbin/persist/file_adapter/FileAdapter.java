...
public class FileAdapter implements Adapter {
  ...
  public void savePolicy(Model model) {
    if (byteArrayInputStream != null && readOnly) {
      throw new CasbinAdapterException(
          "Policy file can not write, because use inputStream is readOnly");
    }
  ...
  private List<String> getModelPolicy(Model model, String ptype) {
    ...
    model
        ...
        .forEach(
            (k, v) -> {
              List<String> p =
                  v.policy.parallelStream()
                      .map(x -> k + ", " + Util.arrayToString(x))
  ...
  private void loadPolicyData(
      Model model, Helper.loadPolicyLineHandler<String, Model> handler, InputStream inputStream) {
    try {
