...
public class DefaultRoleManager implements RoleManager {
  ...
  /**
   * In order to use a specific role name matching function, set explicitly the role manager on the
   * Enforcer and rebuild role links (you can optimize by using minimal enforcer constructor).
   *
   * <pre>
  ...
  public DefaultRoleManager(
      ...
      final BiPredicate<String, String> matchingFunc,
  ...
  private Set<String> getPatternMatchedDomainNames(final String domain) {
    ...
    if (domainMatchingFunc != null) {
      allDomains.keySet().stream()
          .filter(d -> domainMatchingFunc.test(domain, d))
          .forEach(patternDomains::add);
    }
  ...
  /**
   * addLink adds the inheritance link between role: name1 and role: name2. aka role: name1 inherits
  ...
  public void printRoles() {
    allDomains.forEach(
        (domain, roles) -> roles.forEach((name, role) -> Util.logPrint(role.toString())));
  }
}
