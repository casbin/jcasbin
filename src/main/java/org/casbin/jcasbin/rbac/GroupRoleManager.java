 ...
 * but the group information is in the default format (policy start with "g") and the role
 * information is in named format (policy start with "g2", "g3", ...). e.g. p, admin, domain1,
 * data1, read g, alice, group1 g2, group1, admin, domain1
 *
 * <p>As for the previous example, alice should have the permission to read data1, but if we use the
 * DefaultRoleManager, it will return false. GroupRoleManager is to handle this situation.
 */
public class GroupRoleManager extends DefaultRoleManager {
  /**
   * GroupRoleManager is the constructor for creating an instance of the GroupRoleManager
   * implementation.
   *
