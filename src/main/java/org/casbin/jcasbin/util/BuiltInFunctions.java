...
public class BuiltInFunctions {
  ...
  /**
   * KeyGet returns the matched part. For example, "/foo/bar/foo" matches "/foo/*", "bar/foo" will
   * been returned
   *
   * @param key1 the first argument.
   * @param key2 the second argument.
  ...
  /**
   * KeyGet2 returns value matched pattern.For example, "/resource1" matches "/:resource", if the
   * pathVar == "resource", then "resource1" will be returned.
   *
   * @param key1 the first argument.
   * @param key2 the second argument.
   * @return the matched part.
  ...
  /**
   * ipMatch determines whether IP address ip1 matches the pattern of IP address ip2, ip2 can be an
   * IP address or a CIDR pattern. For example, "192.168.2.123" matches "192.168.2.0/24"
   *
   ...
   * @param aviatorEval the AviatorEvaluatorInstance object which contains built-in functions and
   *     custom functions.
   * @return the result of the eval.
   */
  public static boolean eval(
      String eval, Map<String, Object> env, AviatorEvaluatorInstance aviatorEval) {
    boolean res;
    if (aviatorEval != null) {
