...
public class EnforcerUnitTest {
  ...
  public void testEnableAutoSave() {
    ...
    // Because AutoSave is enabled, the policy change not only affects the policy in Casbin
    // enforcer,
    // but also affects the policy in the storage.
    e.removePolicy("alice", "data1", "read");

    // However, the file adapter doesn't implement the AutoSave feature, so enabling it has no
    // effect at all here.
  ...
  public void testGetPermittedActions() {
    Enforcer e =
        new Enforcer(
            "examples/rbac_with_deny_model.conf", "examples/rbac_with_deny_policy.csv", true);
    String sub = "alice";
    String obj = "data1";
  ...
  public void testMultilineModel() {
    Enforcer enforcer =
        new Enforcer("examples/multiline_model.conf", "examples/multiline_policy.csv");
  ...
  public void testPriorityExplicit() {
    Enforcer e =
        new Enforcer(
            "examples/priority_model_explicit.conf", "examples/priority_policy_explicit.csv");
  ...
  public void testBatchEnforce() {
    ...
    List<Boolean> myResults =
        e.batchEnforce(
            asList(
                asList("alice", "data1", "read"),
                asList("bob", "data2", "write"),
                asList("jack", "data3", "read")));
  ...
  public void testBatchEnforceWithMatcher() {
    ...
    List<Boolean> myResults =
        e.batchEnforceWithMatcher(
            matcher,
            asList(
                asList("alice", "data1", "read"),
                asList("bob", "data2", "write"),
                asList("root", "data2", "read"),
                asList("root", "data3", "read"),
                asList("jack", "data3", "read")));
    Assert.assertArrayEquals(myResults.toArray(new Boolean[0]), results.toArray(new Boolean[0]));
  }
